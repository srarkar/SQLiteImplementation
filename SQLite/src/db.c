...
// using strncpy over memcpy ensures all bytes are initialized
...
// simply increment row number to advance
...
// open db file and track size. initialize cache to NULL
Pager* pager_open(const char* filename) {
  ...
  // assert that every node is 1 page
...
// getters and setters for node type
...
// for internal, max key is right
// for leaf, max key is at max index
...
void internal_node_insert(Table* table, uint32_t parent_page_num,
  ...
  *internal_node_num_keys(parent) = original_num_keys + 1;
  ...
  // shift to make room for new cells
...
close db file
frees allocated Pager and Table structs*/
...
// allocate new page to store left side 
...
// search for correct place to store key/value + search for dupes
...
ExecuteResult_t execute_statement(Statement* statement, Table* table) { switch (statement->type) {   
